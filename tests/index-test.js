...
describe(`whyDidYouUpdate`, () => {
  ...
  it(`logs an error on nested props but excludes the parent`, () => {
    ...
    assert.equal(errorStore.entries.length, 4)
    assert.equal(groupStore.entries.length, 4)
    ...
    assert.equal(groupStore.entries[3][0], `Stub.props.a.b.c`)
    ...
    assert.ok(error.test(errorStore.entries[3][2]))
    ...
    assert.deepEqual(logStore.entries[6][2], a.b.c)
    assert.deepEqual(logStore.entries[7][2], a.b.c)
  })
